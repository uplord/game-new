...
func _process(delta):
	if look_ahead_locked:
		target_look_ahead = current_look_ahead
...
func lock_current_look_ahead() -> void:
	look_ahead_locked = true
	enemy_focus_active = false
	enemy_focus_direction = 0.0
	target_look_ahead = current_look_ahead


func lock_look_ahead_direction(dir: float) -> void:
	var locked_direction : float = clamp(dir, -1.0, 1.0)
	look_ahead_locked = true
	enemy_focus_active = false
	enemy_focus_direction = 0.0
	target_look_ahead = locked_direction
	current_look_ahead = locked_direction


func unlock_look_ahead() -> void:
